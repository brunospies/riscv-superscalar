...
architecture structural of DataPath is
    ...
    signal update_en_sig: std_logic;
    ...
    -- =========================
    -- BTB interface signals
    -- =========================
    -- FETCH (IF)
    signal btb_pc_in         : std_logic_vector(31 downto 0);
    signal btb_predict_taken : std_logic;
    signal btb_predict_pc    : std_logic_vector(31 downto 0);
    -- UPDATE (EX)
    signal btb_update_en     : std_logic;
    signal btb_pc_update     : std_logic_vector(31 downto 0);
    signal btb_target_real   : std_logic_vector(31 downto 0);
    --signal btb_taken_real    : std_logic;
    signal btb_predict_taken_ID : std_logic;
...
begin
    ...
    MUX_PC: pc_d <= btb_predict_pc when btb_predict_taken= '1' else
                    branchTarget when (uins_ID.Format = J) or (branch_decision = '1' and bubble_branch_ID = '1') else
                    jumpTarget when uins_ID.instruction = JALR else --jumpTarget(30 downto 0) & '0' when uins_ID.instruction = JALR else 
                    STD_LOGIC_VECTOR(UNSIGNED(pc_ID) + TO_UNSIGNED(4,32)) when branch_decision = '0' and bubble_branch_ID='1' else
                    incrementedPC_IF;
    ...
    -- Stage Instruction Decode of Pipeline
     Stage_ID: entity work.Stage_ID(behavioral)
        port map (
            ...
            branch_taken_in     => btb_predict_taken,
    ...
    BranchDetection_unit: entity work.BranchDetection_unit(arch1)
        port map (
            ...
            branch_prediction  => btb_predict_taken_ID,
            branch_decision    => branch_decision,
            bubble_branch_ID   => bubble_branch_ID
        );
        
        
    -- Branch Target Buffer instance
    BTB_UNIT : entity work.BTB(behavioral)
        port map (
            clock         => clock,
            reset         => reset,
